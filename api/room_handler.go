...
func (h *RoomHandler) HandleGetRooms(c *fiber.Ctx) error {
	rooms, err := h.store.Room.Get(c.Context(), bson.M{})
	if err != nil {
		return err
	}
	return c.JSON(rooms)
...
func (h *RoomHandler) HandleBookRoom(c *fiber.Ctx) error {
	// date format "2025-05-17T00:00:00.0Z"
	...
	if !ok {
		return c.Status(http.StatusInternalServerError).JSON(genricResp{
			Type: "error",
	...
	if !ok {
		return c.Status(http.StatusBadRequest).JSON(genricResp{
			Type: "error",
			Msg:  fmt.Sprintf("room %s already booked, the booking days are %d to %d", c.Params("id"), params.FromDate.Day(), params.TillDate.Day()),
		})
